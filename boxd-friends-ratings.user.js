    ...
    .then((html) =>
      Array.from(
        new DOMParser()
          ...
          .querySelectorAll(".film-rating-group"),
      ).flatMap((section) => {
        const [, score] =
          section
            .querySelector("h2 > .rating")
            ?.className.match(/rated-large-(\d+)/) || [];
        const reviewCount = section.querySelectorAll(
          "ul.avatar-list > li",
        ).length;
                      ...
                      .map((count, index) => {
                        ...
                        return `
                            <li class="rating-histogram-bar" style="width: 15px; left: ${index * 16}px">
                                ${
                                  count > 0
                                    ? `<a href="${ratingLink}" class="ir tooltip" title="${count} ${"★".repeat(stars).concat(stars % 1 == 0 ? "" : "½")} rating${count !== 1 ? "s" : ""}">${stars}★ ${barHtml}</a>`
                                    : `${stars}★ ${barHtml}`
                                }
                            </li>
                        `;
...
const placeHistogram = (histogramHtml, averageRating, user, film, count) => {
  const globalHistogramSection = document.querySelector(
    ...
    friendsHistogramSection.innerHTML = `
            <h2 class="section-heading">
                ...
                <span class="average-rating">
                    <a href="${friendsRatingsLink}" title="${count} rating${count !== 1 ? "s" : ""}">
            ...
            ${histogramHtml}
        `;
    globalHistogramSection.parentNode.insertBefore(
      friendsHistogramSection,
      globalHistogramSection.nextSibling,
    );
  }
};

// Main function to run the script
fetchRatings(username, film).then((ratings) => {
  if (ratings.length) {
    const averageRating = (
      ratings.reduce((sum, rating) => sum + rating, 0) / ratings.length
    ).toFixed(1);
    placeHistogram(
      constructHistogram(ratings, username, film),
      averageRating,
      username,
      film,
      ratings.length,
